...
ID2LABEL = dict(
    enumerate(Path("../data/labels.txt").read_text(encoding="utf8").splitlines())
)
...
def compute_metrics(pred):
    ...
    print(report)
    return {"accuracy": acc, "f1": f1, "classification_report": report}
...
def train():
    ...
    dataset = load_dataset(
        "text", data_files={"train": "data/ner/train.tsv", "test": "data/ner/test.tsv"}
    )
    ...
    def tokenize_function(examples):
        ...
        for input_ids, orig_label in zip(tokenized["input_ids"], labels):
            ...
            assert len(input_ids) == len(
                curr_label
            ), f"{len(input_ids)} != {len(curr_label)}"
